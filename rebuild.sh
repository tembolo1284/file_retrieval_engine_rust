...
# Note: This will only work if the script is sourced, not executed
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
ACTIVATE_PATH="$SCRIPT_DIR/.venv/bin/activate"

# Create helper script to activate environment
cat > activate_env.sh << EOF
#!/bin/bash
source $ACTIVATE_PATH
echo "Python virtual environment activated!"
echo "You can now run:"
echo "  ./file_retrieval_cli.py <command>"
EOF
...
echo ""
echo "Or activate it directly with:"
